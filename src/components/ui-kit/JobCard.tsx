    ...
    <article className={cn(
      ...
      <div>
        <header className="relative flex items-start justify-between gap-3 mb-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            {job.employer.logoUrl ? (
              ...
              <div className={cn(
                "w-10 h-10 rounded-xl flex items-center justify-center text-xs font-black text-white uppercase font-display flex-shrink-0 bg-gradient-to-br shadow-sm",
            ...
            <div className="min-w-0">
              <h3 className="text-base sm:text-lg font-bold text-[var(--ink-900)] leading-snug truncate">
                {job.title}
              </h3>
              <div className="flex items-center gap-1.5 text-xs text-[var(--ink-500)] mt-0.5">
                <span className="truncate font-medium">{job.employer.companyName}</span>
          ...
          <button
            ...
            className="p-2 rounded-xl hover:bg-[var(--ink-100)] press text-[var(--ink-500)] hover:text-[var(--ink-900)] flex-shrink-0 transition-colors"
          >
            <Bookmark size={16} className={cn(job.isSaved && "fill-[var(--brand-pink)] text-[var(--brand-pink)]")} />
          </button>
        </header>
        ...
        <div className="flex flex-wrap items-center gap-1.5 mb-3.5">
          ...
          {job.location && (
            <span className="inline-flex items-center gap-1 text-xs text-[var(--ink-500)]">
        ...
        {(job.salaryMin || job.salaryMax) && (
          <div className="mt-1 mb-4">
            <span className="text-xs text-[var(--ink-500)] block uppercase tracking-wider font-semibold font-sans mb-0.5">Estimated Salary</span>
            <div className="text-lg font-bold text-[var(--ink-900)] tracking-tight leading-none font-sans">
      ...
      <footer className="flex items-center justify-between pt-3.5 border-t border-[var(--ink-300)] mt-auto gap-2">
        <div className="flex items-center gap-1.5 text-xs text-[var(--ink-500)]">
          {job.aiScore != null && (
            <span className="inline-flex items-center gap-1 font-semibold text-primary">
              <Sparkles size={13} /> {job.aiScore}% match
            </span>
          )}
        ...
        <button
          ...
          className={cn(
            "h-8 px-4 rounded-full text-xs font-semibold tracking-wide press transition-all duration-200 flex items-center justify-center shadow-sm",
            job.hasApplied
              ? "bg-[var(--ink-100)] text-[var(--ink-500)] border border-[var(--ink-300)] cursor-not-allowed"
              : "bg-[var(--brand-pink)] text-white hover:bg-[var(--brand-pink-hover)] hover:-translate-y-0.5 hover:shadow-md"
          )}
        >
